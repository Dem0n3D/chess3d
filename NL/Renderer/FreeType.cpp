...
///Create a display list coresponding to the give unsigned character.
void make_dlist ( FT_Face face, unsigned char ch, GLuint list_base, GLuint * tex_base ) {

	//The first thing we do is get FreeType to render our unsigned character
	//into a bitmap.  This actually requires a couple of FreeType commands:

	//Load the Glyph for our unsigned character.
	...
	//the unsigned character has the right amount of space
	//between it and the one before it.
	glTranslatef((GLfloat)bitmap_glyph->left,0,0);

	//Now we move down a little in the case that the
	//bitmap extends past the bottom of the line 
	//(this is only true for unsigned characters like 'g' or 'y'.
	glPushMatrix();
	...
	//the actual unsigned character and store that information in 
	//the x and y variables, then when we draw the
	//quad, we will only reference the parts of the texture
	//that we contain the unsigned character itself.
...
void print(const font_data &ft_font, float x, float y, const char *fmt, ...)  {
	...
	//down by h. This is because when each unsigned character is
	//draw it modifies the current matrix so that the next unsigned character
